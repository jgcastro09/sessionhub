...
import { RegistryEntryView } from './views/RegistryEntryView'
...
// a deep link or by following a Link from ProjectsView, per plan 6.3.
function ProjectScopedContent({ path, tick, onUnauthorized }: { path: string; tick: number; onUnauthorized: () => void }) {
  ...
  if (params) return <RegistryEntryView projectId={params.projectID} entryId={params.entryID} tick={tick} onUnauthorized={onUnauthorized} />

  params = matchPath('/projects/:projectID/registry', path)
  if (params) return <RegistryView projectId={params.projectID} tick={tick} onUnauthorized={onUnauthorized} />
